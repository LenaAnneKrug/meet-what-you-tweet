package com.whimisicalbees.blm

import com.whimisicalbees.nlp.SentimentAnalyzer
...
import org.carrot2.core.{ Document, ControllerFactory }
import org.carrot2.clustering.lingo.LingoClusteringAlgorithm

import com.danielasfregola.twitter4s.entities.{ Tweet, LocationTrends, GeoCode, Accuracy }
...
object TweetWhatYouMeet extends App {
	...
	val woeid_nyc = 2459115 //Hard coded this but it is based on Yahoo!s Where On Earth Identifier (https://en.wikipedia.org/wiki/WOEID)
	val nyc_geocode = GeoCode.apply(40.7259940, -73.9957090, new Accuracy(8000, Measure.Meter)) //Hard Coded for exeriment
	 ...
	 * takes the first 25 unique users and gets there recent tweets and process them using a Lingo Clustering Algorithm to determin 
	 * Categories
	 */
		...
		Future.sequence(trends.trends.subList(0, 10).map(trend => doSearch(trend.name))).onComplete {
			...
			case Success(results) => {
				...
				Future.sequence(users.map(user => userTimeline(user))).onComplete {
					case Success(tweets) => {
						tweets.filter(t => !t.isEmpty).foreach(t => {
	...
	def usersFromTweets(tweets: Seq[Tweet]) = {
		...
		tweets.filter(!_.user.isEmpty).map(tweet => tweet.user.get.screen_name).distinct
	}

	/*
	 * Processes tweets using a Lingo Clustering Algorith,
	 */
